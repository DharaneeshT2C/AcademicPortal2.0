...
export default class CopBulkImport extends LightningElement {
  @api recordId;
  ...
  @wire(getCourseOfferingNamesForTerm, { academicTermId: "$recordId" })
  wiredOfferings({ data }) {
    this.offeringOptions = data || [];
    this.validateRows();
  }

  @wire(getParticipationStatuses)
  wiredParticipationStatuses({ data }) {
  ...
  get importDisabled() {
    return !this.recordId || !this.rows.length || this.hasValidationErrors;
  }
  ...
  validateRows() {
    ...
    for (let i = 0; i < this.rows.length; i += 1) {
      ...
      if (!offeringName) {
        ...
        errors.push({ key: `offering-invalid-${i}`, message: `Row ${rowNumber}: Course Offering "${offeringName}" is not under this Academic Term.` });
      }
