...
import startExamTermResultImport from "@salesforce/apex/HistoricalDataImportController.startExamTermResultImport";

const PROCESS_NAME = "Exam Term Result Import";

export default class ExamTermResultImporter extends LightningElement {
  file;
  rows = [];
  headers = [
  ...
  get helpExampleRow() {
    const sample = {
      ...
      GPA: "8.25",
      CGPA: "8.11",
      "Total Grade Points": "24.75",
      "Credits Earned": "3",
      Status: "Active",
    };
    return this.helpExampleHeaders.map((header, index) => ({
      key: `exam-term-help-${index}`,
      value: sample[header] || "",
    }));
  }
  ...
  async handleImport() {
    ...
    const confirmed = await LightningConfirm.open({
      ...
      message: `Start importing ${this.rows.length} exam term result row${this.rows.length === 1 ? "" : "s"}?`,
      theme: "warning",
    });
    if (!confirmed) return;
    ...
    startExamTermResultImport({
      ...
      .then((jobId) => {
        ...
        this.startPolling();
        this.showToast("Import started", "Exam term result batch job queued.", "info");
      })
      .catch((error) => {
  ...
  handleDownloadTemplate() {
    ...
    this.downloadCsv(`\uFEFF${content}`, "exam_term_result_template.csv");
  }
  ...
  handleDownloadFailed() {
    ...
    this.downloadCsv(`\uFEFF${lines.join("\n")}`, "exam_term_result_failed_rows.csv");
  }
