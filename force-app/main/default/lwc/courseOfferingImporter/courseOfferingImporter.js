...
export default class CourseOfferingImporter extends LightningElement {
  ...
  get importDisabled() {
    return !this.selectedAcademicTermId || !this.rows.length || this.validationErrors.length > 0;
  }
  ...
  loadSessions() {
    getAcademicSessionsForTerm({ academicTermId: this.selectedAcademicTermId })
      .then((data) => {
        this.sessions = data || [];
        this.validateRows();
      })
  ...
  validateRows() {
    ...
    for (let i = 0; i < this.rows.length; i += 1) {
      ...
      if (!sessionName) {
        ...
        errors.push({ key: `session-invalid-${i}`, message: `Row ${rowNumber}: Academic Session "${sessionName}" is not under the Academic Term for the selected Exam Cycle.` });
      }
      if (!courseCode) {
