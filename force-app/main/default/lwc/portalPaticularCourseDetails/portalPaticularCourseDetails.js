...
export default class PortalPaticularCourseDetails extends LightningElement {
  ...
  initializeScreenData() {
    if (this.selectedPathwaysOnly === true) {
      this.loadLearnerPathways()
        ...
        .catch((error) => {
          // eslint-disable-next-line no-console
          console.error("portalPaticularCourseDetails init error", error);
        })
        .finally(() => {
  ...
  loadCoursesFromSelectedTemplates() {
    ...
    if (!templateIds.length) {
      return getCoursesForSemester({
        ...
        .catch((error) => {
          // eslint-disable-next-line no-console
          console.error("portalPaticularCourseDetails Apex Error", error);
          this.processCourses([]);
        });
    }
  ...
  handleCancelFlow() {
    ...
    const currentPagePrefix = currentPath.replace(/\/courseparticulardetails(?:\/.*)?$/i, "");
