...
import getAcademicSessionsForTerm from "@salesforce/apex/KenCourseOfferingBulkImportController.getAcademicSessionsForTerm";
import startImport from "@salesforce/apex/KenCourseOfferingBulkImportController.startImport";
import getImportErrors from "@salesforce/apex/KenCourseOfferingBulkImportController.getImportErrors";
import getBatchStatus from "@salesforce/apex/KenImportUtils.getBatchStatus";

export default class CourseOfferingBulkImport extends LightningElement {
  @api recordId;

  headers = ["Academic Session", "Course Code", "Section Number", "Course Type", "Faculty Email", "Enrollment Capacity", "Waitlist Capacity", "Start Date", "End Date"];
  rows = [];
  parsedHeaders = [];
  sessions = [];
  validationErrors = [];
  ...
  @wire(getAcademicSessionsForTerm, { academicTermId: "$recordId" })
  wiredSessions({ data }) {
    this.sessions = data || [];
    this.validateRows();
  }
  ...
  get importDisabled() {
    return !this.recordId || !this.rows.length || this.validationErrors.length > 0;
  }
  ...
  get helpRows() {
    const sampleRows = [
      {
        "Academic Session": "Semester 1",
        "Course Code": "MIT001",
        "Section Number": "1",
        "Course Type": "Lecture",
        "Faculty Email": "faculty@example.edu",
    ...
    return sampleRows.map((row, rowIndex) => ({
      key: `help-${rowIndex}`,
      cells: this.headers.map((header, cellIndex) => ({
        key: `help-cell-${rowIndex}-${cellIndex}`,
        value: row[header] || "",
      })),
    }));
  }
  ...
  async handleImport() {
    ...
    const confirmed = await LightningConfirm.open({
      ...
      message: `Start importing ${this.rows.length} course offering row${this.rows.length === 1 ? "" : "s"}?`,
      theme: "warning",
    });
    if (!confirmed) {
      return;
    ...
    startImport({ jsonRows: JSON.stringify(this.rows), academicTermId: this.recordId })
      .then((jobId) => {
        this.jobId = jobId;
        this.jobStatus = "Queued";
        this.totalRecords = this.rows.length;
        this.startPolling();
        this.showToast("Import started", "Course Offering bulk import job queued.", "info");
      })
      .catch((error) => {
  ...
  validateRows() {
    const errors = [];
    const sessionNames = new Set((this.sessions || []).map((session) => this.normalize(session.name)));
    const seenKeys = new Set();

    for (let i = 0; i < this.rows.length; i += 1) {
      const row = this.rows[i];
      const rowNumber = row.RowNumber || String(i + 1);
      const sessionName = row["Academic Session"];
      const courseCode = row["Course Code"];
      const courseType = row["Course Type"];

      if (!sessionName) {
        errors.push({ key: `session-required-${i}`, message: `Row ${rowNumber}: Academic Session is required.` });
      } else if (sessionNames.size > 0 && !sessionNames.has(this.normalize(sessionName))) {
        errors.push({ key: `session-invalid-${i}`, message: `Row ${rowNumber}: Academic Session "${sessionName}" is not under this Academic Term.` });
      }
      if (!courseCode) {
        errors.push({ key: `course-required-${i}`, message: `Row ${rowNumber}: Course Code is required.` });
      }
      if (!courseType) {
        errors.push({ key: `type-required-${i}`, message: `Row ${rowNumber}: Course Type is required.` });
      }

      const duplicateKey = `${this.normalize(sessionName)}|${this.normalize(courseCode)}|${this.normalize(courseType)}`;
      if (sessionName && courseCode && courseType) {
        if (seenKeys.has(duplicateKey)) {
          errors.push({ key: `duplicate-${i}`, message: `Row ${rowNumber}: Duplicate Academic Session, Course Code, and Course Type.` });
        }
  ...
  handleDownloadTemplate() {
    const sampleRows = [
      ["Semester 1", "MIT001", "1", "Lecture", "faculty@example.edu", "60", "10", "2026-06-01", "2026-12-01"],
      ["Semester 1", "MIT001", "1", "Practical", "faculty@example.edu", "30", "5", "2026-06-01", "2026-12-01"],
    ];
    const lines = [this.headers.join(",")].concat(sampleRows.map((row) => row.map((value) => this.csvCell(value)).join(",")));
    this.downloadCsv(`\uFEFF${lines.join("\n")}`, "course_offering_bulk_import_template.csv");
  }
  ...
  handleDownloadFailed() {
    ...
    this.downloadCsv(this.buildFailedCsv(), "course_offering_bulk_import_failed_rows.csv");
  }
